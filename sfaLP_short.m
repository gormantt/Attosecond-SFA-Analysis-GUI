classdef sfaLP_short < sfaLPsolver
...
%% Methods %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
function obj = sfaLP_short(sfaParam)
% Constructor
    ...
    obj.type            =   'Short';
    obj.t0_m            =   (.5*pi-sfaParam.phi)/sfaParam.omega;
    obj.tr_m            =   obj.t0_m;
    ...
    % Reference values for future sfa computations
        ...
        if any(isnan(freq))
            ...
            if (freq(ind(1)) < sfaParam.Ip) || (freq(ind(1)) > obj.kappa*sfaParam.Up+sfaParam.Ip)
                error('sfaLP:sfaLP_short:RefFreq','Out-of-bound reference frequencies - does not overlap with the threshold to cut-off energy range');
            end
...
function [t0,tr,p] = saddlePointSolution (obj,sfaParam,Freq)
    ...
    if isempty(obj.t0_ref)
        ...
        error('sfaLP:sfaLP_short:NoRefInit','Strong-field approximation solver has not been initialized with the appropriate reference(s)')
    end
