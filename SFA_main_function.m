function [] =SFA_main_function(Imin,Imax,Istep,Ip,fitmin,fitmax,filter_name,filter_thickness,det_gas)

    %Title: SFA_main_function
    %Author: Tim Gorman
    %This function loads RABBITT data created by Dietrich's RABBIT script and it
    %subtracts a filter delay, an atomic delay from detection or, neither then
    ...
    if length(FileName)>1 %checks whether or not files were selected
        ...
        if iscell(FileName)==true %there are multiple files
            ...
            [FileName_sorted,lambda_sorted]=sort_files_by_wavelengths(FileName); %sort file names according to wavelength in RABBITT file
