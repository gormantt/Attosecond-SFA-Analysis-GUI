function [exported_info,exported_data] =SFA_main_function_v3(Imin,Imax,Istep,Ip,fitmin,fitmax,filter_name,filter_thickness,det_gas,fix_int_check_val)
    ...
    %v2... I forget one I changed here. I think file export
    %v3... allows for choosing a fixed intensity.  This added an input to
    %SFA_main_function_v3.  This GUI also has an updated intensity_fit
    %function.
    ...
    if length(FileName)>1 %checks whether or not files were selected
        ...
        if iscell(FileName)==true %there are multiple files
            ...
            for i=1:length(FileName)
                    ...
                    [E_GD,GD_sub,GD_exp_err,lambda,I0_m]=intensityFit_v5(loopfile,Imin,Imax,Istep, Ip, fitrange,filt_phase,at_delay_mcc,det_Ip,fix_int_check_val); %this is where the fitting is done
